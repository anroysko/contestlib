...
class EulerTourTree {
	private:
		struct Treap {
			...
			Treap(bool r, Treap* nxt) : pri(rand<int>()), real(r) {
				if (nxt) {
					pre = nxt->pre;
					nxt->pre = this;
				} else pre = this;
	...
	public:
		...
		EulerTourTree(int n) : nodes(n, nullptr) {
			for (auto& ptr : nodes) ptr = new Node(1, nullptr);
		}

		~EulerTourTree() {
			for (int i = 0; i < nodes.size(); ++i) {
				for (Treap* cur = nodes[i]->pre; cur != nodes[i]; ) {
					Treap* tmp = cur;
					cur = cur->pre;
					delete tmp;
				}
